...
class RSRuby
  ...
  def method_missing(r_id, *args)
    ...
    robj_name = self.class.convert_method_name(r_id.to_s)
    ...
    if args.length > 0
      ...
      lcall_args = self.class.convert_args_to_lcall(args)
  ...
  def self.convert_method_name(name)
  ...
  #   => [['',1],['',2],['',3],['a',4],['b',5]]
  def self.convert_args_to_lcall(args)
    lcall_args = []
  ...
  def self.set_default_mode(m)
    if m < -1 or m > TOP_CONVERSION
      raise ArgumentError, "Invalid mode requested"
    end
    instance.default_mode = m
  end

  # Returns the current default conversion mode as an Integer.
  # DEPRECATED: Use the accessor on the RSRuby instance isntead
  def self.get_default_mode
    instance.default_mode
  end

  # TODO - not implemented
  def self.set_rsruby_input(m)
    @@rsruby_input = m
  end

  # TODO - not implemented
  def self.get_rsruby_input
    @@rsruby_input
  end

  # TODO - not implemented
  def self.set_rsruby_output(m)
    @@rsruby_output = m
  end

  # TODO - not implemented
  def self.get_rsruby_output
    @@rsruby_output
  end

  # TODO - not implemented
  def self.set_rsruby_showfiles(m)
    @@rsruby_showfiles = m
  end

  # TODO - not implemented
  def self.get_rsruby_showfiles
    @@rsruby_showfiles
  end
